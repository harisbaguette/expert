...
const dir=process.argv[2]||'/tmp/solo-material-use';fs.mkdirSync(dir,{recursive:true});
(async()=>{const browser=await pp.launch({headless:true});try{
 const page=await browser.newPage();
 const svg=fs.readFileSync(path.join(root,'docs/images/expert-definition/solo-material-use.svg'),'utf8');
 ...
 if(!process.env.FAST_RENDER){
  await page.screenshot({path:path.join(dir,'solo-material-use.png'),fullPage:true});
