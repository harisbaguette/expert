...
M10은 접속·도구 호출·결과 반환과 작업별 격리 공간의 실행을 소유한다. M31은 권한, M32는 위험·비밀·격리 정책과 준수 검사, M33은 손실 한도를 소유한다. M10은 그 정책을 받아 파일·네트워크·자원 제한을 실행 공간에 적용한다. 정책을 통과했어도 실제 격리가 실패하면 실행을 보류한다. 전문 작업의 내용과 방법은 M09·M38·M52–M54, 결과 검증은 M42·M46에 연결한다.
