...
현재 구축·대체 판정은 화면·문서·시스템·통신 안에서 끝나는 업무에 적용한다. 제공된 현장 기록을 분석할 수 있으나 새 실측·시공·시술·촬영·실물 인도를 수행하는 경로는 현재 범위에 포함하지 않는다. 본문 설명과 세부 책임의 연결은 [대응표](materials.md#본문과-세부-책임의-대응)를 따른다.
...
직무 양산의 단위는 **공통 정의의 재사용 + 직무 정의의 구체화 + 개별 위임의 적용**이다. 공통 정의는 재료와 인터페이스, 직무 정의는 대상·방법·도구·자산·평가 조건, 개별 위임은 당사자·목표·권한·자료·기한을 담는다. 내용을 참조·확장하되 최종 적용값과 출처·판본을 확인할 수 있어야 한다. 직무 수만큼 코드나 규칙 원문을 복제할 필요는 없다.

재료의 기본 사용 방식은 [상시·단계별·조건부](materials.md#재료의-적용-구분)로 설명한다. 실제 조립에서는 적용 여부·활성 조건·수행 시점을 각각 기록한다. 조건 확인 책임을 유지하는 것과 도구를 매번 호출하는 것을 구별한다. 하위 책임·지원·증거 상태는 [하위 책임 계약](materials.md#하위-책임과-지원-증거)을 따른다.

| 조립할 때 정할 것 | 계약 |
|---|---|
| 업무 범위 | 세부 업무·행위·대상·난도·관할·운영 환경·외부 의존을 명시한다. 직무 이름 하나를 모든 업무 지원의 표시로 쓰지 않는다 |
| 전문성 | 핵심 객체·관계·방법·전제·질문·판단 지점·개입·반증·예외를 M21·M22·M34·M35에 연결한다. 지식 원문·실행 방법·재사용 자산을 구별한다 |
| 입출력·실행 | 각 요구를 입력·판단·도구/외부 수행·산출 또는 상태 변화·검사·인수에 연결한다. 기능마다 형식·버전·환경·권한·라이선스·자원 조건을 확정한다 |
| 조건부 기능 | 활성 조건, 확인할 자료, 의존, 대체 경로, 종료·재개 조건을 가진다. 참이면 필수, 거짓이면 비활성, 불명이면 필요한 확인과 영향받는 경로의 대기로 처리한다 |
| 의존과 충돌 | 재료·도구·자산·외부 수행의 필수 의존을 끝까지 연결한다. 대체 경로는 하나를 골라도 요구를 충족해야 한다. 순환은 공동 계산·반복·중단 조건을 정의하며 목표·권한·판본 충돌은 명시적으로 해결한다 |
| 재사용·변경 | 공통·직무·개별 건의 판본과 참조를 기록한다. 고객 자료·비밀·특수 권한을 공통 정의로 승격하지 않는다. 원천·환경·관계가 바뀌면 관련 조건·의존·검사·진행 건을 갱신한다 |

조립 결과에는 적용된 재료·담당 책임·판본·의존, 요구별 실행·검사 경로, 활성·비활성·미확인 조건, 부족한 내용·환경·외부 증거가 남는다. 요구된 동작을 소유한 재료가 없거나 의존·충돌이 해결되지 않았으면 정의는 미완성이다. 정의가 완성돼도 구현·실증의 상태는 별도로 유지한다. 비활성 기능은 현재 경로를 막지 않지만 활성화 전에 필수 조건을 충족해야 한다. 미완성 경로와 무관한 작업까지 일괄 중지하지 않는다.

조건을 판정할 조회·관측·진단·실험은 본 기능의 실행 의존과 따로 연결해 먼저 준비한다. 그 진단도 자체 권한·비용·위험 조건을 충족해야 한다. 진단 허용은 본 실행의 허가가 아니며, 진단 결과가 불명이거나 진단을 할 수 없으면 대체 근거·기한·대기 경로를 남긴다. 조건 확인에 필요한 기능을 그 조건이 참인 뒤에만 켜도록 연결한 순환 대기를 조립 검사에서 검출한다.

조건은 직무명·단어 목록으로 고정 판정하지 않는다. 모델이 요구와 자료의 의미를 판단하고, 구조화할 수 있는 버전·단위·권한·필수 참조·지원 조건은 검사기로 확인한다. 모델이 새 의존을 발견하면 명세·계획을 갱신한다. ‘조건부’라는 표기만으로 미지의 상황을 해결했다고 처리하지 않는다.

여러 직역은 상태와 요구를 함께 연결한다. 예를 들어 형사·민사·회계의 문서가 같은 거래를 다룰 때 공통 사실 참조는 공유하되 당사자별 비밀·주장·전문 판단은 구분한다. 합의·교육·상담·운영처럼 파일이 목적이 아닌 업무도 상대 반응·이해·행동·외부 상태를 산출과 검증에 포함한다.

부족한 내용은 M50이 기존 재료 안의 지식·방법·도구·자산 보완으로 우선 해결한다. 새 재료는 기존 책임으로 맡길 수 없는 입출력·판정이 있을 때만 추가한다. 공통 정의를 고치면 영향받는 직무의 조합을 다시 검사하고, 새 조합의 검증 없이 기존 기능의 성적을 물려주지 않는다.

## 수행 범위와 새 상황의 처리

M35는 실행 계획을 만들 때 필요한 전문 판단·입력 해석·도구 작업·출력·검사 능력을 나열하고 현재 지원 증거와 대조한다. 지원은 직무명 하나가 아니라 `세부 업무·대상·난도·관할·입출력 형식·작업 종류·모델/도구/환경 판본·자료 조건`의 범위로 기록한다. 구현 상태(planned·implemented), 검증 상태(unverified·passed·failed·stale), 이번 업무의 수행 가능 판정은 각각 관리한다. 과거의 verified 표시는 구현과 증거를 분리해 이관한다.

| 확인 결과 | 다음 담당과 행동 | 다음 단계로 갈 근거 |
|---|---|---|
| 해당 범위의 능력·권한·환경·필수 검사 확인 | M35가 M09·M38·M52–M54·M56 중 실제 작업 담당에 배정 | 유효한 지원 증거와 실행 조건 |
| 새로운 조합이나 낯선 조건 | M18·M21·M22·M34가 차이와 결론을 바꿀 조건을 분석하고 필요한 조회·진단·시험 수행 | 기존 방법의 적용 근거 또는 새 검증. 낯설다는 이유만으로 무조건 사람에게 넘기지 않음 |
| 지식·방법·도구의 구체적인 부족 발견 | M50이 확보 출처·변경 대상·기대 효과·검증 과제를 정해 보완하고 M49가 시험 | 새 작업의 검사·기존 회귀·권한·비용 조건을 충족한 범위만 지원으로 갱신 |
| 충분한 전문 근거나 실행 수단 확보 실패 | M39에 대안·시도·남은 판단·기한을 준비해 해당 경로를 이관 | 필요한 판단·권한·수단이 실제 확보됨. 미지원 결과를 완성품으로 보고하지 않음 |
| 목적·조건 자체가 함께 성립하지 않음 | M34가 제약의 충돌과 가능한 범위·대안을 제시 | M28–M31의 유효한 범위 변경 또는 달성 불가 판정. 목표를 몰래 낮추지 않음 |

직무별 지식은 핵심 객체·용어·단위·방법·적용 전제·예외·숙련 사례·반대 사례·주요 판단 지점·필요 증거·치명적 오류·허용 결과·전문가 비교 과제로 채운다. 파일 개수나 지식 문장 수로 충족을 선언하지 않는다. 서식·원문은 M55·M12–M18, 방법은 M21·M22, 이번 계획은 M35가 소유한다. 분야 이름만 바꾼 동일한 일반 지침을 모든 직무의 전문 지식으로 세지 않는다.

미지의 문제는 모델의 자기 확신 하나로 판정하지 않는다. M18의 반대 근거·자료 불일치, M42의 예상/관측 차이, M43의 독립 반증, M44의 목록 밖 위험 탐색, M49의 보호된 새 과제, M51의 지연 성과를 별도 신호로 사용한다. 실패·거절·불명·전문 개입을 평가에서 빼지 않으며 검사하지 못한 범위가 결론에 미치는 영향을 남긴다. 모든 미지의 위험을 발견할 수 있다고 주장하지 않는다.

## 복합 업무의 상태·전략·협력

### 문제 정의와 결정의 계약

M34는 의뢰인의 표현을 그대로 할 일로 바꾸기 전에 원하는 성과·방해 원인·대안 설명·절대 제약·불필요한 작업을 구별한다. 입력은 M17의 사실과 M38의 질문·응답, 방법의 근거는 M21·M22다. 출력에는 문제 정의, 목표별 우선순위와 결정권, 현재 가설과 반대 가설, 결론을 바꿀 증거, 가능한 대안과 전환 조건을 남긴다. 목표·권한·보수·기한 변경은 M28·M31의 유효한 변경 절차를 거친다. 빈칸을 채우기 위해 의미 없는 질문이나 확률을 만들지 않는다.

M35는 이 결정을 요구별 작업·도구·검사·인수에 연결한다. 계산은 M53, 새 관측은 M54, 상대와의 상호작용은 M38이 수행한다. M45는 문제를 잘못 정의했는데 실행만 정확한 사례를 검사한다. M43·M44는 제시한 대안 밖의 반례도 찾는다. 자료의 부재, 모형의 오차, 선택의 불확실성, 외부 결과의 우연을 서로 다른 불확실성으로 남긴다.
...
현장 실측·증언·진료·기관 판단·실물 시험은 모델의 추론이나 렌더로 대체되지 않는다. 현재 업무에 제공된 외부 관측 자료는 취득 조건·품질·기한을 확인하고, 접근할 수 없으면 해당 사실은 미확인으로 남긴다. 새로운 현장 수행이 반드시 필요하면 그 업무는 현재 구축 범위 밖으로 기록한다. 외부 수행자의 전문 분석과 자격 행위, 단순 자료 전달을 구분해 사람 기여와 총비용에 기록한다.
...
실행 가능한 기능은 `기능 + 형식 + 프로그램/버전 + 작업 종류 + 환경`으로 구분하고 지원·미지원·미검증 조건과 시험 증거를 남긴다. 재료의 구현 상태(planned·implemented)와 검증 상태(unverified·passed·failed·stale)는 별도로 관리하며, 상태 표시만으로 이 기능별 지원 범위를 대신하지 않는다. 읽기 지원을 편집 지원으로, 신규 생성 지원을 원본 보존 지원으로 확대하지 않는다.
...
규칙은 법·의무 기준, 직업 윤리·의무, 에이전트 기본 원칙, 계약·합의 조건, 플랫폼·제출처 규정, 회사 규칙, 사용자 지정 규칙으로 구분한다. 본문 항목과 세부 책임은 [대응표](materials.md#본문과-세부-책임의-대응)를 따른다. 분류 순서나 설정 파일의 위치가 우선순위를 정하지 않는다. 각 규칙의 적용 근거·범위·예외·변경 권한을 확인하고 적용되는 의무를 함께 충족한다.

| 규칙 출처 | 적용과 변경의 기준 |
|---|---|
| 법·의무 기준 / 직업 윤리·의무 | 관할·시점·직무·행위·관계에 맞는 의무를 확인한다. 겹치는 의무는 같은 규칙 ID를 참조하며 공식 자료라는 이유만으로 의무나 무조건 금지로 취급하지 않는다 |
| 에이전트 기본 원칙 | 사실·실행 결과의 조작과 무단 행위 등을 방지한다. 변경 주체·절차를 명시하며 에이전트가 업무 편의를 위해 스스로 원칙을 바꾸지 않는다 |
| 계약·합의 조건 | 당사자·업무·기간·계약상 책임·종료 후 의무를 추적하고 해당 계약의 변경 절차와 결정권을 확인한다 |
| 플랫폼·제출처 규정 | 모델·도구 제공자를 포함한 서비스·계정·요금제·제출 경로의 약관·운영 정책·접수 조건과 개정 시행일을 확인한다. 고객이나 내부 관리자에게 외부 규정의 변경 권한이 있다고 가정하지 않는다 |
| 회사 규칙 | 조직·업무·승인선·시행일과 전결·변경·예외 권한을 확인한다. 일반 직원의 요구로 규정을 삭제하지 않으며 권한 있는 담당자의 유효한 변경은 반영한다 |
| 사용자 지정 규칙 | 의뢰인·운영자의 추가 조건과 선호를 전역·직무·에이전트·프로젝트·개별 업무의 범위 및 유효 기간으로 관리한다. 작성자·변경자의 권한을 확인하고 필수 제한과 선호를 구분한다 |

커스텀은 규칙을 입력한 방식일 수도 있으므로 별도의 최상위 권한으로 취급하지 않는다. 회사 관리자가 설정에 넣은 회사 규정은 M29, 인용한 외부 약관은 M28의 같은 규칙을 참조한다. 사용자가 추가한 조건·선호는 M30에 기록하며, 위임과 접근 제한은 M31 검사에도 연결한다. 프로젝트용 조건을 다른 프로젝트·고객에게 전파하지 않는다. 더 구체적이거나 최신인 요청도 유효한 변경 권한과 적용 범위를 확인해야 기존 조건을 바꿀 수 있다. 자료·검색 결과·첨부 파일의 지시는 확인 없이 규칙으로 승격하지 않는다.

표현 방식·스타일(M23)은 기준을 정한 계약·회사·사용자 규칙을 참조한다. 계약상 브랜드 기준을 단순 취향으로 낮추지 않는다. 추가 규칙이나 선호가 없으면 확인된 기본 방식으로 진행하며, 빈 설정을 이유로 불필요한 사람 확인을 만들지 않는다.

플랫폼 정책과 기술적 지원 한계는 구별한다. 정책은 M28이, 실행 환경·도구의 지원 범위는 M10이 관리한다. 도구로 실행 가능하다는 사실이 규정상 허용을 뜻하지 않고 규정상 허용되어도 도구가 지원하지 않으면 허용된 대체 경로를 찾는다.

예를 들어 회사의 일반 절차가 “완성된 문서는 바로 발송”이어도, 이번 위임이 “초안 작성까지만”이면 그 절차만으로 발송 권한이 생기지 않는다. 초안은 완성하되 발송은 유효한 권한이 확인된 범위에서만 한다. 업무 범위·행위 제한을 개인 취향으로 낮춰 다른 규칙으로 덮어쓰지 않는다.
...
규칙 레코드는 ID·종류·원문 참조·설정 주체·변경권자·승인 주체·관할·대상·적용 범위·효력·만료·판본·필수 조건/선호 구분·해석 근거·구조화된 조건·행동 제한·예외·철회 상태를 가진다. 변경은 원문 근거와 권한을 확인하고 M20·M25로 진행 중 업무의 영향과 기존 승인의 유효성을 다시 확인한다. 자연어를 실행 규칙으로 옮길 때 해석 후보와 근거를 검증한다. 확정되지 않은 법적 해석을 조건식으로 포장해 자동 통과시키지 않는다.
...
모든 대기는 `wait_id`, 원인, 막힌 요구·행동, 담당, 인증할 응답 주체, 깨울 신호, `deadline`, 재확인 시점, 횟수·비용 한도, 만료 시 행동, 독립적으로 계속할 작업을 가진다. 장기 성과의 대기는 성과 확인일까지 유지하고 주기적 점검 시점을 둔다. 아무 신호 없이 다시 질문하거나 무한 상향하지 않는다. 마지막 결정권자도 응답하지 않으면 다음 사람을 지어내지 않는다.

| 사건 | 처리 |
|---|---|
| 유효한 응답·변경 신호 도착 | 중복·순서·주체·현재 권한을 확인한 뒤 상태를 다시 읽고 영향받는 단계로 재개 |
| 응답 없음, 대기 기한 전 | 정한 주기에만 재확인·독촉. 무관한 작업은 계속하고 필요한 비용·노출은 유지 |
| 기한 만료, 대체 경로 있음 | 기존 권한·완료 기준을 지키는 경로로 재계획. 목표 축소는 유효한 범위 변경으로만 처리 |
| 기한 만료, 대체 경로 없음 | 차단된 범위를 미완료·중단으로 기록하고 남은 위험·후속 담당을 전달. 원래 배정·비용·지연은 평가에 남김 |
| 취소·권한 철회·늦은 승인 | 미실행 작업 중단. 이미 실행한 외부 효과는 현재 허용 권한으로 대사. 만료된 업무·승인을 자동으로 부활시키지 않음 |
| 납품 후 무응답 | 사전에 합의한 객관적 인수 조건이 충족됐다면 그 근거로 인수한다. 명시 승인이 필요한 경우 침묵을 승인으로 바꾸지 않음 |

이 계약은 자료 요청·의도 확인·승인·전문가 이관·도구 대사·인수·후속 성과에 공통 적용한다. 사실 질문, 계약상 선택, 전문 판단 지원은 이관 사유와 사람 기여를 구별해 기록한다.

## 도구와 외부 실행

M10은 접속·도구 호출·결과 반환과 작업별 격리 공간의 실행을 소유한다. M31은 권한, M32는 위험·비밀·격리 정책과 준수 검사, M33은 손실 한도를 소유한다. M10은 그 정책을 받아 파일·네트워크·자원 제한을 실행 공간에 적용한다. 정책을 통과했어도 실제 격리가 실패하면 실행을 보류한다. 전문 작업의 내용과 방법은 M09·M38·M52–M54·M56, 결과 검증은 M42·M46에 연결한다.
...
M56은 거래·주문·접수·게시·운영 설정처럼 외부 상태를 바꾸는 업무를 소유한다. M35가 목적과 실행·복구 순서를 정하고 M56이 실제 대상·내용·전제를 확정한다. M10은 호출과 응답을 전달하며 업무 성공을 스스로 판정하지 않는다. M31·M32·M33의 권한·제약·예산, M25의 상태·판본, M42의 결과 검증, M40의 인수와 연결한다.

행동마다 외부 계정·업무 ID·행동 ID·대상·입력 내용과 해시·기준 판본·전제·승인 조건·실행 소유자/세대·멱등 키와 유효 기간·최대 노출·예상 결과·조회 방법·복구/보상 조건을 연결한다. 검사한 대상과 실행 대상의 동일성을 실행 직전까지 확인한다. 외부 시스템이 조건부 변경이나 실행 세대 검사를 지원하지 않으면 그 한계를 기록하고 변경 경쟁·중복 실행 위험에 맞는 허용 범위를 정한다. 내부 검사만으로 외부의 정확히 한 번 실행을 보장하지 않는다.

| 처리 상태 | 다음 행동 | 예산·완료 처리 |
|---|---|---|
| 실행 전 | 대상·입력·권한·기준 판본 확인, 실행 의도 저장, 노출 예약 후 호출 | 예약 실패나 조건 불일치면 호출하지 않음 |
| 성공 확인 | 영수증·실제 대상·내용을 M42로 대조하고 M25 기록 | 실제 비용을 정산하고 결과를 M40에 인계 |
| 실행되지 않은 실패 확인 | 원인·권한·내용·전제가 여전히 같은지 확인해 재시도 또는 재계획 | 발생하지 않은 노출만 해제. 재시도도 현재 조건을 검사 |
| 처리 불명 | 조회·원천 대사만 수행하고 확인 기한·담당 유지 | 외부 노출 예약을 유지. 시간 초과만으로 실패나 미실행을 확정하지 않음 |
| 일부 성공 | 성공한 단계·미완료 단계·남은 약속을 기록하고 남은 실행 또는 허용된 보상 수행 | 전체 작업을 다시 보내지 않음. 실제 잔여 손실과 복구 비용 유지 |
| 취소·승인 철회 | 아직 나가지 않은 실행을 중단하고 이미 나간 결과를 최소 권한으로 조회 | 취소가 과거 효과를 없앴다고 보지 않음. 보상은 별도 권한 확인 |
| 보상 성공 / 실패·불명 | 보상 효과와 남은 손실을 다시 검증하고 미해결 경로는 담당·기한과 연결 | 보상 성공도 원래 목표의 성공과 구분. 실패·불명은 노출과 기록을 남김 |

멱등 키가 만료되거나 내용·판본이 바뀌면 기존 불명 행동을 새 키로 다시 보내지 않는다. 이전 행동의 결과·잔여 효과를 대사한 뒤 실제로 별개의 행동인지 확인한다. 읽기·대사에도 현재 허용된 최소 권한이 필요하며, 권한 철회는 자동으로 새 조회 권한을 만들지 않는다.

새 쓰기와 보상·정정은 각각 권한과 예산을 확인한다. 보호나 원상 복구를 명분으로 더 큰 손실·정보 반출·무권한 행동을 허용하지 않는다. 미해결 거래를 남긴 채 프로세스만 종료할 때도 업무 기록은 해당 상태와 담당·기한을 보존한다.
...
모든 새 요청·예약·외부 신호·기회 발견은 발신자·수신 범위·중복·순서를 확인하고 긴급도와 기한을 평가한다. 진행 중 사고는 원래 요청의 형태와 관계없이 현재 권한·한도 안의 보호 조치로 연결한다. 조치 자체에도 실행 직전 검사·외부 결과 확인·기록이 필요하다. 새 승인 없이 허용되는 보호 조치가 없으면 미승인 쓰기를 수행하지 않고 대기·통지·허용된 관측을 유지한다.

기존 건을 불러오면 목표 변경 여부만 보지 않는다. 사실·주장·자료 판본·규정·당사자 관계·권한·도구·환경·기한·예산·미확정 외부 행동·의존 요구의 변경을 M25로 대조한다. 조회 실패를 변경 없음으로 판정하지 않는다. 권한 철회는 실행 관문에, 새 증거는 자료·가설과 관련 판단에, 환경 변경은 지원·검증 증거에 각각 전파한다. 무관한 유효 결과는 재사용한다.
...
외부 행동 상태는 M56의 성공 확인·미실행 실패 확인·처리 불명·일부 성공·취소·보상 상태로 따로 관리한다. 업무 전체의 상태와 단일 도구 응답을 같은 칸에 넣지 않는다. 처리 불명은 대사 전 새 쓰기를 막으며, 일부 성공은 성공분을 보존해 남은 범위만 처리한다. 재시도·재계획·복구·취소에도 예산·기한·권한을 다시 적용한다.
...
입력은 실제 고객이 주는 PDF·스캔·표·이미지·영상·음성·손글씨·현장 사진·메시지를 포함한다. 구조가 있는 자료는 값·수식·연결·위치를 뽑고, 구조가 없는 자료는 내용을 해석해 장면·구간과 함께 남긴다. 중요 숫자·날짜·이름·단위는 독립 추출이나 원천 대조로 확인하고, 값마다 원본 위치와 읽기 불확실성을 보존한다. 못 읽은 값을 지어내지 않고 재취득·대체 원천·추가 질문을 시도한다. 지원하지 않는 형식은 읽기 수단 확보나 형식 변환으로 먼저 잇고, 그 경로가 모두 막힐 때만 사람에게 넘긴다.
...
기한에는 원문 표현·기산 사건·관할 또는 업무 달력·시간대·마감 시각·적용 판본을 붙인다. 날짜만 있는 값을 임의의 자정 시각으로 바꾸거나 영업일을 단순 경과 시간으로 계산하지 않는다. 시간대·휴일·마감 변경과 시계 차이가 실행에 미치는 영향을 확인하며, 예약·재시도 간격과 대외 기한을 구별한다. 의미가 불명확하면 결론을 바꿀 기준을 확인한 뒤 계산한다.
...
M57은 에이전트 프로세스 밖에서 수행 기반을 감시하고 장애 조정·복구·재개를 소유한다. M02는 실행 순환, M03은 저장 복원, M06은 입력 수신, M56은 외부 실행 대사, M32는 안전 경계, M48은 원인 분석을 맡는다. 감시 경로 자체의 누락도 별도 heartbeat·원천 대사·비상 담당으로 확인한다. 업무상 기회와 마감의 의미 판단은 M37에 남긴다.

복구 뒤 새 쓰기를 허용하려면 저장 상태·수신 기록·외부 영수증·승인 및 철회·활성 정책·현재 실행 세대·예약 예산이 맞는지 확인한다. 오래된 작업자를 그대로 깨우거나 대체 모델의 접속 성공만으로 전문 능력이 유지됐다고 선언하지 않는다. 장애 때 영향받지 않은 작업은 유지하되 공유 장애의 영향 범위를 먼저 확인한다.
...
원치 않은 실행은 M57이 사고 상태로 접수하고 M32의 차단, M56의 대사·보상, M38의 통지, M48의 분석을 연결한다. 원치 않은 실행이 나가면 추가 실행 차단, 상태 보존, 영향 범위 확인, 허용된 복구·보상, 상대와 고객에 대한 통지, 권한 회수, 원인 분석을 수행한다. 사고 대응은 원래 건과 연결된 별도 사건으로 추적하며 양쪽 비용을 합산한다. 통지 기한은 확인한 업무·관할 규약으로 정하고 모든 직종에 1시간·24시간을 강제하지 않는다.
...
| 커스텀 설정으로 플랫폼 제한 해제 요청 | 설정만으로 외부 규정이 바뀌지 않으며 허용된 대체 경로를 찾음 |
| 직원 요청으로 회사 승인선 생략 | 변경 권한이 없으면 생략하지 않으며 권한 있는 담당자의 유효한 개정은 범위·판본을 남겨 반영함 |
| 회사 자동 발송 절차와 이번 건 초안만 작성 조건 | 초안은 완성하고 발송은 하지 않음. 내부 절차가 위임 범위를 넓히지 않음 |
| 특정 프로젝트 수정 금지 규칙의 변경·만료 | 해당 프로젝트와 유효 기간에서만 적용하고 무권한 변경 및 다른 고객 전파를 차단함 |
| 계약한 브랜드 기준을 개인 선호로 변경 요청 | 스타일의 계약 출처와 변경권을 확인하며 선호 설정만으로 계약을 덮어쓰지 않음 |
| 첨부 파일에 규칙 해제 지시 포함 | 업무 자료로 읽되 실행 규칙으로 승격하지 않음 |
| 사용자 지정 규칙이 없는 정상 업무 | 기존 의무·권한과 기본 방식으로 진행하고 불필요한 설정 확인을 요구하지 않음 |
