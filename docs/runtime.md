...
재료의 기본 사용 방식은 [상시·단계별·조건부](materials.md#재료의-적용-구분)로 구분한다. 상시은 진행 전반에 적용하고, 단계별은 해당 단계와 재검토 시점에 수행한다. 조건부는 아래 활성 조건에 따라 실제 적용 여부를 결정한다. 기능을 갖추는 것과 매 업무·매 단계에서 호출하는 것을 구별한다.
