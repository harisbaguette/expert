...
실행 제어는 정해진 계획에 따라 함수·도구·AI를 호출하고 순차·병렬 실행, 조건 분기, 결과 합류, 일부 실패·대기·취소·재개를 처리한다. 선행 결과의 준비 여부와 다음 입력의 형식·단위·의미 조건을 확인한다. 값·상태로 정해진 계산·조회·변환·분기는 모델을 거치지 않고, 의미 판단이 필요한 곳만 모델에 맡긴다. 모델이 최종 답변을 냈다는 사실은 실행 턴의 종료일 뿐 업무 성공의 증거가 아니다. 완료는 미리 정한 조건을 외부 증거로 확인한 뒤 판정한다.

대안·전략 판단이 방법을 고르고, 업무 절차·실행 계획이 작업·연결·담당·일정을 정한다. 작업 진행·연결은 그 계획 안에서 지금 가능한 업무를 다음 담당·검토·수정으로 잇고, 실행 제어는 실제 호출과 대기를 수행한다. 현재 진행 상태에는 선택한 분기·병렬 결과·계획 변경 이력을 남긴다. 계획 밖의 변경은 계획·전략 판단으로 돌려보낸다. 자세한 설명은 [정의 2](<../전문가 에이전트 정의 2.md#판단과-실행의-역할>)를 따른다.
...
재료의 기본 사용 방식은 [상시·단계별·조건부](materials.md#재료의-적용-구분)로 구분한다. 상시는 진행 전반에 적용하고, 단계별은 해당 단계와 재검토 시점에 수행한다. 조건부는 아래 활성 조건에 따라 실제 적용 여부를 결정한다. 기능을 갖추는 것과 매 업무·매 단계에서 호출하는 것을 구별한다.
...
M35는 실행 계획을 만들 때 필요한 전문 판단·입력 해석·도구 작업·출력·검사 능력을 나열하고 현재 지원 증거와 대조한다. 지원은 직무명 하나가 아니라 `세부 업무·대상·난도·관할·입출력 형식·작업 종류·모델/도구/환경 판본·자료 조건`의 범위로 기록한다. 구현 상태(planned·implemented·verified)와 이번 업무의 수행 가능 판정은 다르다.
...
충돌 시에는 정의 1에서 확정한 법적 적용 관계를 먼저 반영한다. 그다음 직업상 필수 의무·에이전트 기본 원칙·승인된 보호 기준 → 유효한 계약·플랫폼 필수 규정 → 조직의 필수 규칙 → 사용자 지정 규칙 → 기본 방식·스타일 순으로 검토한다. 문서 이름이나 저장 순서가 아니라 실제 의무의 근거와 설정 권한으로 분류하며, 충돌을 푼 뒤에도 남는 조건과 보호 기준을 함께 충족한다.
...
업무 지식(M21)은 개념·대상·원리·방법과 관계의 의미, 적용 조건·예외·이견을 관리한다. 자료 확보(M07·M19·M18) → 대상·개념의 분류·결합(M52) → 내용 종합·문서 작성(M09, 필요한 대화는 M38) → 근거·결과 검증(M42·M43) → 검증된 기록 반영(M56)으로 이어진다. 작업 진행·연결(M41)이 수정·반영까지 관리하며, 새 재료 ID는 만들지 않는다.

지식의 주장·관계마다 원문 위치·판본·적용 시점·접근 권한을 연결한다. 같은 이름의 다른 대상을 합치거나 근거 없는 관계를 만들지 않았는지, 예외·이견·관련 참조가 빠지지 않았는지 검사한다. 잘못된 결합은 되돌릴 수 있도록 이전 연결과 변경 이유를 남긴다.

변경 감시·반영(M20)이 원문의 정정·철회를 알리면 관련 지식과 파생 문서·진행 건·검사를 따라 재검토 대상을 찾는다. 확인 중인 내용은 확정 지식과 구분하며 검증에 실패한 수정안을 반영하지 않는다. 사실 정정은 근거 대조 후 반영하고, 수행 능력에 영향을 주는 지식·방법·도구 변경은 M50과 M49의 개선·능력 시험으로 연결한다. 고객별 장기 기억을 공통 지식으로 옮길 때도 같은 검사와 재사용 권한 확인을 거친다.
...
| 시나리오 | 확인할 결과 |
|---|---|
| 함수만 쓰는 흐름과 함수·AI 혼합 흐름 | 정해진 값·상태 분기를 코드로 처리하고 의미 판단이 필요한 작업만 모델에 연결함 |
| 병렬 일부 실패·필수 결과 누락·취소 후 재개 | 성공 결과와 선택 분기를 보존하고 합류 조건을 검사함. 누락된 필수 결과 없이 후속 실행하지 않음 |
| 원문 정정·관계 오류·지식 참조 단절 | 관련 지식과 파생 결과를 찾아 재검토하고, 수정·검증 전 상태를 확정 지식으로 사용하지 않음 |
