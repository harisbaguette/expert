...
정의 문서 계통별 재료 55개(환경 9 · 자료·검색 11 · 지식·경험 2 · 기억·상태 2 · 규칙·권한 9 · 행동 체계 5 · 실무 7 · 검증·평가 8 · 학습·개선 2)를 한 줄씩 업계 구현과 맞춰 봄. 결론: **조사 당시 업계 부품 중 55개 재료 어디에도 안 들어가는 것은 0개로 판단했다.** 재료의 필요성 대조와 계통 배치 검토는 별도다. 아래 표의 계통은 2026-09-11 재배치에 맞췄고 업체별 조사 내용은 당시 기록을 유지했다. 반대로 업계에 구현 사례가 없는 재료는 2개(시험 환경·실험·시뮬레이션)인데, 되돌릴 수 없는 행동 앞에 필요한 것이라 빼지 않음. materials.md에는 여러 모델 비교·고객 교차 차단·협상 책임이 있으며, 계통별 책임 경계는 [배치 검토](../reviews/재료/material-system-placement-review.md)에 따라 보완했다.
...
| 자료·검색 | 자료·구조 읽기 | 손글씨·체크박스 추출(EvenUp), 사실/의견 구분(Supio), Legal OCR(BHSN) | — |
...
| 규칙·권한 | 정체성·작풍 | Preference Learning 팀 문체(Spellbook), 의뢰인별 Custom Memory(Crosby) | — |
...
| 실무 | 데이터 처리 | 추출 → 임베딩 → 색인, 타입 있는 중간 형식(Harvey UDF), 청킹(Eve) | — |
| 실무 | 계산·분석·최적화 | 의료비·손해 계산(EvenUp·Eve 연표 → 비용 목록) | 약함 |
| 실무 | 실험·시뮬레이션 | **사례 없음** — Crosby 협상 시뮬레이션은 연구 단계 | 유지 |
| 실무 | 산출물 제작·편집 | 추적변경 병합, 3-way 병합 특허(Ironclad), 결정론 .docx 내보내기(Eve) | — |
| 실무 | 상대 응대 | 상대 AI 와 4라운드 협상(Luminance), Jenny 이의 처리·따뜻한 넘김 | M38 협상 조항이 덮음 |
| 실무 | 보고·납품·인수 | review-ready 산출물, Portal 로 의뢰인에게(Legora), CMS 에 다시 씀(Eve) | — |
...
| 검증·평가 | 고정 시험 | 야간 카나리·버전 붙인 평가 데이터·leave-one-out 게이트(Harvey), CoCoBench 회귀 | — |
| 검증·평가 | 성과 추적 | 생산 모니터링(Harvey), NPS + 피드백 채널(LBOX), 가드레일 99~99.99% 목표(Crosby) | — |
...
## 5-2. 업체별로 다른 것 — 같은 재료를 어떤 기술로 채웠나

재료 목록은 다 같고, 회사마다 다른 건 **어느 재료에 돈을 걸었나 + 그 재료를 무슨 기술로 풀었나** 둘임. 나머지 재료는 남의 것(모델 API·SDK·판례 DB)을 빌려 씀.

| 회사 | 승부 재료 | 그 재료를 푼 기술 | 빌려 쓰는 것 |
|---|---|---|---|
| Harvey | 검색 · 검증·평가 · 나눠 맡기기 | 자체 법률 임베딩 + 문단 메타데이터 + LLM 관련도 판정 / 주장 분해 → 출처 대조, 변호사 채점표 벤치 3종 / 규칙 1개 = 하위 에이전트 1개 후 추적변경 병합 / 오픈 모델 덧학습(Tenet) | 판례 DB(LexisNexis), 에이전트 루프(OpenAI Agent SDK), 모델 4사 |
| Legora | 행동 체계(완료 조건) · 권한 | 목표 대비 자체 평가 후 되돌아가 반복, 노코드 빌더 거부 / Zanzibar 권한 + 문서 권한 상속 그래프 / 인용·근거 점수 따로 매기는 BAR 벤치 | 모델(Claude·Azure OpenAI), 법령·판례 DB 6곳(+Qura 인수) |
| CoCounsel (TR) | 자료·검색 · 사람에게 넘기기 | 자체 판례 DB + KeyCite, 혼합 색인 → 재정렬 → 토큰 정렬 특허 / 결정 지점마다 멈추는 Brief Builder / 별도 Verify 패스 | 에이전트 뼈대(Claude Agent SDK) |
| Lexis Protégé | 자동 갱신 · 나눠 맡기기 | Shepard's At Risk(인용 판례가 뒤에 뒤집혔나 추론) / 지휘자 + 조사·웹·고객문서 에이전트 3개 / 모델 사용자 선택 또는 Best Fit | 모델(Claude·GPT·o3) |
| Luminance | 상대 응대 · 장기 기억 | 상대 AI 와 Word 첨부로 라운드 협상, 사람은 hard stop 만 / 전사 계약 기관 기억 / 자체 LPT 모델 | (자체 모델 주장, 검증 불가) |
| Ironclad | 판단 규칙 · 학습 | 수락·거부를 허용·차단 목록으로(특허) / 충돌 편집 3-way 병합(특허) / 8개 역할 에이전트 + Manager | 모델(Claude·ChatGPT MCP) |
| Spellbook | 개인 취향 · 회사 규칙 | 수락 문구만 1년 보관해 다음 제안과 비교 / 규칙 = 기본 입장 + 대안 + 위험도, 통과·실패 표시 / 자동 수락 없음 | 모델 미공개 |
| EvenUp | 자료·구조 읽기 · 결과 검증 | 손글씨·체크박스 추출 자체 미세조정(Piai) / 사람 100~150명 전수 재작성 / Opus 설계 → Sonnet·Haiku 작성 | 모델(Claude·OpenAI) |
| Eve | 시작 신호 · 먼저 감지하기 · 안전장치 | 기록 도착 즉시 발동(Atlas), 사건 상태 바뀌면 자동 시작 / 야간 전체 순찰 Auditor 6종 / 결정론 코드가 LLM 출력 검증, stop_reason 이어 쓰기 / 검증된 판례 DB 만 | 모델(Anthropic API) |
| Supio | 출처·시점·판본 · 업무 지식 | 사실(경찰 보고)과 의견(의사 소견) 구분 / 사건 유형 112개 렌즈 / 사람 전수 검증 | 모델 미공개 |
| Crosby | 지식·경험 · 학습 | 변호사가 직접 프롬프트 작성, 손 라벨 조항 5만 / 변호사 옆 엔지니어 + RL 미세조정 / 부위별 신뢰 점수로 약한 곳만 사람 | 모델 3사 |
| Garfield | 업무 처리 흐름 · 사람에게 넘기기 | 소액 채권 절차를 끝까지 잇되 매 단계 의뢰인 승인 / 규제기관(SRA) 인가 안에서 인가 변호사 책임 | 모델 미공개 |
| LBOX | 나눠 맡기기 · 결과물 평가 | 쟁점별 검색 도구 병렬 호출 / 판결문을 논술 문제 + 채점표로 바꿔 추론 과정까지 채점 | 모델 미공개 |
| BHSN | 모델 | 자체 법률 LLM(계속 사전학습 + 변호사 RLHF), Legal OCR 특허 | 바탕 모델 미공개 |
| 슈퍼로이어 | 검색 | 요청마다 function calling 으로 자료 주문 + 인용 적절성 평가 | 모델(Claude 3.5 Sonnet) |

읽는 법: 자체 기술이 들어간 칸은 **검색(임베딩·색인) · 검증(주장 분해·판례 생사) · 학습(수락 목록·RLHF) · 권한(Zanzibar·벽)** 넷에 몰려 있음. 모델·에이전트 루프·판례 DB 는 18개 중 15개가 빌려 씀. 우리가 직접 만들 것도 이 넷이고, 나머지는 빌리는 게 업계 표준임.
